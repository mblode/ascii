...
export const normalizeVectors = (
  ...
  const maxValues: number[] = new Array(dimensions).fill(0);
  ...
  const normalizedCharacters: AsciiCharacter[] = characters.map(
    (character) => ({
      ...
      vector: character.vector.map((value, i) => {
        ...
        return maxValues[i] > 0 ? value / maxValues[i] : 0;
      }),
    })
  );
 ...
 */
export const getCharacterSetCacheSize = (): number => {
  return characterSetCache.size;
};
